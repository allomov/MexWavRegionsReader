...
RegionWaveReader::RegionWaveReader(){
	file = new QFile();
}

RegionWaveReader::RegionWaveReader(QString name)
{
    file = new QFile(name);
}

QString RegionWaveReader::readChunkName(){
    return RiffFileDefs::readString(file);
}

int RegionWaveReader::readChunkSize(){
    int chSize = RiffFileDefs::readInt(file);
    if (chSize==-1) return chSize;
    return chSize%2==0 ? chSize : (chSize+1);
}

void RegionWaveReader::read(){
...
MarkerList* RegionWaveReader::getMarkers() {
    return markerList;
}

RegionList* RegionWaveReader::getRegions() {
    return regionList;
}

void RegionWaveReader::errorMessage(QString s) {
    errors.append(s);
}

bool RegionWaveReader::hasErrors(){
    return !errors.empty();
}

QString RegionWaveReader::getLastError(){
    return errors.last();
}

void RegionWaveReader::setFileName(QString s){
